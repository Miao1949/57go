...
func main() {
	numberOfOptions, minLen, numberOfSpecialCharacters, numberOfDigits := getInput()
	...
	for i := 0; i < numberOfOptions; i++ {
		...
		fmt.Println(generatePassword(minLen, numberOfSpecialCharacters, numberOfDigits, randomNumberGenerator))
	}
}

func generatePassword(minLen int, numberOfSpecialCharacters int, numberOfDigits int, randomNumberGenerator *rand.Rand) (password string) {
	...
	password = string(randomizedRuneArray)
	return
}
...
func getInput() (numberOfOptions int, minLen int, numberOfSpecialCharacters int, numberOfDigits int) {
	numberOfOptions = getPositiveNumber("Number of options? ")
