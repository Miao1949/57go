...
func calculateTax(orderAmount uint64, state string, county string) (total float64, tax float64) {
	...
	wisconsinTaxMap := make(map[string]float64)
	wisconsinTaxMap[EauClaire] = EauClaireCountyExtraTax
	wisconsinTaxMap[Dunn] = DunnCountyExtraTax

	if state == Wisconsin || state == WisconsinAbbreviation {
		taxRate = WisconsinTax
		taxRate += wisconsinTaxMap[county]
